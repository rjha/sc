...
a.params.action="ADD";
a.endPoint="/monitor/action/item/feature.php";
webgloo.sc.SimplePopup.init();
webgloo.sc.SimplePopup.post(a,{dataType:"json",reload:false,onSuccess:function(){$("#fps-"+a.params.postId).html("*featured");
}});
});
...
webgloo.sc.groups={addPanelEvents:function(){$("#add-group-btn").click(function(d){d.preventDefault();
var f=jQuery.trim($("#group-box").val());
if(f==""){return;
}var e=f.split(",");
for(var b=0;
b<e.length;
b++){var a=jQuery.trim(e[b]);
if(a==""){continue;
}var c=' <li> <input type="checkbox" name="g[]" checked ="checked" value="'+a+'"/>'+a+"</li> ";
$(".group-panel .wrapper ul:first").append(c);
}$("#group-box").val("");
});
$("a#uncheck-all-groups").click(function(a){a.preventDefault();
$(".group-panel").find(":checkbox").removeAttr("checked");
});
$("a#check-all-groups").click(function(a){a.preventDefault();
$(".group-panel").find(":checkbox").attr("checked","checked");
});
},addCloudBox:function(){$(".fancy-box").fancybox({"type":"iframe","width":"75%","height":"75%"});
}};
