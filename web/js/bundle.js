...
return b&&{end:(function(){var e="TransitionEnd";
if(a.browser.webkit){e="webkitTransitionEnd";
}else{if(a.browser.mozilla){e="transitionend";
}else{if(a.browser.opera){e="oTransitionEnd";
}}}return e;
}())};
})();
});
...
a.prototype={constructor:a,close:function(j){var h=c(this),f=h.attr("data-target"),g;
if(!f){f=h.attr("href");
f=f&&f.replace(/.*(?=#[^\s]*$)/,"");
}g=c(f);
g.trigger("close");
...
a.prototype={constructor:a,setState:function(f){var h="disabled",c=this.$element,e=c.data(),g=c.is("input")?"val":"html";
f=f+"Text";
...
b.prototype={cycle:function(){this.interval=setInterval(a.proxy(this.next,this),this.options.interval);
return this;
...
},slide:function(f,e){var c=this.$element.find(".active"),d=e||c[f](),k=this.interval,h=f=="next"?"left":"right",j=f=="next"?"first":"last",g=this;
this.sliding=true;
k&&this.pause();
d=d.length?d:this.$element.find(".item")[j]();
if(d.hasClass("active")){return;
}if(!a.support.transition&&this.$element.hasClass("slide")){this.$element.trigger("slide");
...
this.$element.one(a.support.transition.end,function(){d.removeClass([f,h].join(" ")).addClass("active");
...
a.fn.carousel=function(c){return this.each(function(){var f=a(this),e=f.data("carousel"),d=typeof c=="object"&&c;
...
},show:function(){var f=this.dimension(),c=a.camelCase(["scroll",f].join("-")),e=this.$parent&&this.$parent.find(".in"),d;
...
},hide:function(){var c=this.dimension();
...
return this;
},transition:function(g,d,e){var f=this,c=function(){if(d=="show"){f.reset();
}f.$element.trigger(e);
};
...
!function(d){var b='[data-toggle="dropdown"]',a=function(f){var e=d(f).on("click.dropdown.data-api",this.toggle);
d("html").on("click.dropdown.data-api",function(){e.parent().removeClass("open");
});
};
a.prototype={constructor:a,toggle:function(k){var j=d(this),f=j.attr("data-target"),h,g;
if(!f){f=j.attr("href");
f=f&&f.replace(/.*(?=#[^\s]*$)/,"");
}h=d(f);
h.length||(h=j.parent());
...
}d.fn.dropdown=function(e){return this.each(function(){var g=d(this),f=g.data("dropdown");
...
!function(e){var a=function(j,h){this.options=h;
this.$element=e(j).delegate('[data-dismiss="modal"]',"click.dismiss.modal",e.proxy(this.hide,this));
};
...
d.call(this);
c.call(this,function(){var j=e.support.transition&&h.$element.hasClass("fade");
!h.$element.parent().length&&h.$element.appendTo(document.body);
h.$element.show();
if(j){h.$element[0].offsetWidth;
}h.$element.addClass("in");
j?h.$element.one(e.support.transition.end,function(){h.$element.trigger("shown");
}):h.$element.trigger("shown");
});
},hide:function(j){j&&j.preventDefault();
if(!this.isShown){return;
}var h=this;
this.isShown=false;
...
function g(){var h=this,j=setTimeout(function(){h.$element.off(e.support.transition.end);
f.call(h);
},500);
this.$element.one(e.support.transition.end,function(){clearTimeout(j);
f.call(h);
});
}function f(h){this.$element.hide().trigger("hidden");
c.call(this);
}function c(l){var k=this,j=this.$element.hasClass("fade")?"fade":"";
if(this.isShown&&this.options.backdrop){var h=e.support.transition&&j;
this.$backdrop=e('<div class="modal-backdrop '+j+'" />').appendTo(document.body);
if(this.options.backdrop!="static"){this.$backdrop.click(e.proxy(this.hide,this));
}if(h){this.$backdrop[0].offsetWidth;
}this.$backdrop.addClass("in");
h?this.$backdrop.one(e.support.transition.end,l):l();
...
this.$backdrop=null;
}function d(){var h=this;
if(this.isShown&&this.options.keyboard){e(document).on("keyup.dismiss.modal",function(j){j.which==27&&h.hide();
});
}else{if(!this.isShown){e(document).off("keyup.dismiss.modal");
}}}e.fn.modal=function(h){return this.each(function(){var l=e(this),k=l.data("modal"),j=e.extend({},e.fn.modal.defaults,l.data(),typeof h=="object"&&h);
if(!k){l.data("modal",(k=new a(this,j)));
}if(typeof h=="string"){k[h]();
}else{if(j.show){k.show();
}}});
};
e.fn.modal.defaults={backdrop:true,keyboard:true,show:true};
e.fn.modal.Constructor=a;
e(function(){e("body").on("click.modal.data-api",'[data-toggle="modal"]',function(m){var l=e(this),j,h=e(l.attr("data-target")||(j=l.attr("href"))&&j.replace(/.*(?=#[^\s]+$)/,"")),k=h.data("modal")?"toggle":e.extend({},h.data(),l.data());
m.preventDefault();
h.modal(k);
});
});
...
b.fn.popover.defaults=b.extend({},b.fn.tooltip.defaults,{placement:"right",content:"",template:'<div class="popover"><div class="arrow"></div><div class="popover-inner"><h3 class="popover-title"></h3><div class="popover-content"><p></p></div></div></div>'});
}(window.jQuery);
...
this.refresh();
this.process();
}a.prototype={constructor:a,refresh:function(){this.targets=this.$body.find(this.selector).map(function(){var c=b(this).attr("href");
return/^#\w/.test(c)&&b(c).length?c:null;
});
this.offsets=b.map(this.targets,function(c){return b(c).position().top;
});
},process:function(){var f=this.$scrollElement.scrollTop()+this.options.offset,e=this.offsets,c=this.targets,g=this.activeTarget,d;
for(d=e.length;
d--;
){g!=c[d]&&f>=e[d]&&(!e[d+1]||f<=e[d+1])&&this.activate(c[d]);
}},activate:function(d){var c;
this.activeTarget=d;
...
b(function(){b('[data-spy="scroll"]').each(function(){var c=b(this);
c.scrollspy(c.data());
});
});
}(window.jQuery);
...
a.prototype={constructor:a,show:function(){var g=this.element,e=g.closest("ul:not(.dropdown-menu)"),d=g.attr("data-target"),f,c;
if(!d){d=g.attr("href");
d=d&&d.replace(/.*(?=#[^\s]*$)/,"");
}if(g.parent("li").hasClass("active")){return;
}f=e.find(".active a").last()[0];
g.trigger({type:"show",relatedTarget:f});
c=b(d);
this.activate(g.parent("li"),e);
this.activate(c,c.parent(),function(){g.trigger({type:"shown",relatedTarget:f});
});
...
this.shown=false;
return this;
},lookup:function(e){var d=this,c,f;
this.query=this.$element.val();
if(!this.query){return this.shown?this.hide():this;
}c=a.grep(this.source,function(g){if(d.matcher(g)){return g;
}});
c=this.sorter(c);
...
},highlighter:function(c){return c.replace(new RegExp("("+this.query+")","ig"),function(d,e){return"<strong>"+e+"</strong>";
});
...
c.preventDefault();
},keypress:function(c){if(!this.shown){return;
...
}if(webgloo.sc.ImageSelector.num_selected==0){webgloo.sc.ImageSelector.showError("Please select an image first.");
return false;
...
}if(webgloo.sc.ImageSelector.num_added>0){$("#step2-container").fadeIn("slow");
...
},processUrlFetch:function(c){images=c.images;
...
var b="Total {total} images found";
var d=b.supplant({"total":webgloo.sc.ImageSelector.num_total});
